...
export function PigmentMixPreview({ pigments, mixResult }: PigmentMixPreviewProps) {
  ...
  return (
    <div className="mix-preview">
      <div className="mix-preview__header">
        ...
        <span className="mix-preview__license">Mixbox © Secret Weapons · CC BY-NC 4.0</span>
      </div>

      {hasBothPigments && mixResult ? (
        <div className="mix-preview__body">
          <div className="mix-preview__swatch" style={{ backgroundColor: mixResult.hex }} />
          <div className="mix-preview__details">
            ...
            <p className="mix-preview__ratio">
              {pigments.A?.name} {Math.round(mixResult.ratioA * 100)}% · {pigments.B?.name}{' '}
              {Math.round(mixResult.ratioB * 100)}%
            </p>
          </div>
        </div>
      ) : (
        <p className="mix-preview__placeholder">Select pigments A and B to preview their Mixbox blend.</p>
      )}
    </div>
  )
}
